...
export default function Landing({ onGetStarted }: LandingProps) {
  ...
  return (
    <div className={darkMode ? 'min-h-screen bg-slate-950 text-slate-50' : 'min-h-screen bg-slate-50 text-slate-900'}>
      ...
      <nav className={(darkMode
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className={
                (darkMode
                  ...
                  : 'text-slate-700 hover:text-slate-900') + ' px-3 sm:px-4 py-2 text-sm font-medium transition-colors hidden sm:block'
              }
            >
            ...
            <button
              onClick={onGetStarted}
              className="px-4 sm:px-5 py-2 bg-emerald-600 text-white text-sm rounded-lg font-semibold hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-600/30"
            >
              Get Started
            </button>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:py-12 md:py-16 grid md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] gap-8 md:gap-10 items-stretch">
        {/* Left: Copy + CTA */}
        <section className="flex flex-col justify-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-emerald-500/40 bg-emerald-500/10 text-xs font-medium text-emerald-300 mb-4 sm:mb-5 w-fit">
            <Sparkles className="w-3 h-3" />
            <span>AI-assisted grant discovery</span>
          </div>

          <h1 className="text-3xl sm:text-4xl lg:text-6xl font-extrabold tracking-tight mb-3 sm:mb-4">
          ...
          <p className={
            ...
            ' text-base sm:text-lg lg:text-xl leading-relaxed max-w-xl mb-5 sm:mb-6'
          }>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center mb-5 sm:mb-6">
            <button
              onClick={onGetStarted}
              className="px-6 sm:px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-xl shadow-emerald-600/30 text-base sm:text-lg text-center"
            >
              Start Matching Me (Free)
            </button>
            <p className={(darkMode ? 'text-slate-400' : 'text-slate-500') + ' text-sm text-center sm:text-left'}>
              Free: 5 matches / month · No card
            </p>
          </div>

          {/* Stats - Responsive grid */}
          <dl className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 max-w-2xl mt-3 sm:mt-4">
            {stats.map((item) => (
              <div
                key={item.label}
                className={
              ...
              >
                ...
                <dd className="text-lg sm:text-xl font-semibold">{item.value}</dd>
              </div>
            ))}
          </dl>
        </section>
        ...
        <section
          className={
            ...
            ' rounded-2xl border p-5 flex flex-col justify-between min-h-[300px] sm:min-h-[320px]'
          }
        >
          <div className="mb-4">
          ...
          <div className="space-y-3 text-sm">
            <div className={(darkMode ? 'bg-slate-800' : 'bg-slate-100') + ' rounded-xl p-3 flex justify-between items-start gap-3'}>
              <div>
                <p className="font-semibold text-sm sm:text-base">NEA Small Grants for Organizations</p>
                <p className={darkMode ? 'text-slate-400 text-xs' : 'text-slate-600 text-xs'}>
                  Arts & culture · Up to $25,000 · National
                </p>
              </div>
              <span className="px-2 py-1 rounded-full bg-emerald-500/15 text-emerald-400 text-[11px] font-semibold whitespace-nowrap">
                Best fit
              </span>
            </div>
            ...
            <div className={(darkMode ? 'bg-slate-800/80' : 'bg-slate-100') + ' rounded-xl p-3 flex justify-between items-start gap-3'}>
              <div>
                <p className="font-semibold text-sm sm:text-base">State Arts Recovery Micro‑grants</p>
                <p className={darkMode ? 'text-slate-400 text-xs' : 'text-slate-600 text-xs'}>
                  Your state · Rolling · $2,500–$10,000
                </p>
              </div>
              <span className="px-2 py-1 rounded-full bg-slate-500/10 text-slate-300 text-[11px] whitespace-nowrap">
                Easy win
              </span>
            </div>
          ...
          <button
            onClick={onGetStarted}
            className="mt-4 sm:mt-5 w-full py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold transition-colors"
          >
            See my matches
          </button>
        </section>
      ...
      <footer className={
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs sm:text-sm">
          <div className="flex flex-col sm:flex-row items-center gap-3 text-center sm:text-left">
            ...
            />
            <div>
              <p className={darkMode ? 'text-slate-400' : 'text-slate-600'}>Grant Geenie — Find grants you qualify for.</p>
          ...
          <p className={darkMode ? 'text-slate-600' : 'text-slate-500'}>
            Made for small nonprofits, artists & founders.
          </p>
        </div>
      </footer>
    </div>
