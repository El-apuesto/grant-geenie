...
export default function Settings({ onBack, onRestartTour, onRetakeQuestionnaire }: SettingsProps) {
  ...
  const handleRetakeQuestionnaire = async () => {
    ...
    try {
      // Reset onboarding_completed so the questionnaire shows again
      ...
      // Reload the page to trigger questionnaire
      window.location.reload();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to reset questionnaire');
    }
  };
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 mb-6">
        ...
        <p className="text-slate-300 mb-4">
          Want to update your full eligibility profile? Retake the 8-question questionnaire to refine your grant matches based on your current needs.
        </p>
        <button
